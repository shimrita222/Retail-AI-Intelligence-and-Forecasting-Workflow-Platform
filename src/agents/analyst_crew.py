...
def compute_micro_inspection(df: pd.DataFrame) -> dict[str, Any]:
    ...
    # 3. Extreme markdown outliers (IQR method per markdown column, record-level)
    ...
    for col in ("MarkDown1", "MarkDown2", "MarkDown3", "MarkDown4", "MarkDown5"):
        ...
        outliers = df[df[col] > upper_bound][["Store", "Dept", "Date", col]].sort_values(col, ascending=False)
    ...
    # 4. Date-level anomalies: extreme week-over-week swings within a Store/Dept series
    df_sorted = df.sort_values(["Store", "Dept", "Date"]).copy()
    df_sorted["prev_sales"] = df_sorted.groupby(["Store", "Dept"])["Weekly_Sales"].shift(1)
    df_sorted["pct_change"] = (
        (df_sorted["Weekly_Sales"] - df_sorted["prev_sales"]) / df_sorted["prev_sales"].abs()
    )
    anomalies = df_sorted.dropna(subset=["pct_change"])
    anomalies = anomalies.reindex(anomalies["pct_change"].abs().sort_values(ascending=False).index)
    top_anomalies = anomalies.head(ANOMALY_TOP_N)[
        ["Store", "Dept", "Date", "prev_sales", "Weekly_Sales", "pct_change"]
    ].copy()
    top_anomalies["Date"] = top_anomalies["Date"].astype(str)
    findings["date_level_anomalies"] = top_anomalies.round(3).to_dict(orient="records")
...
<div class="section">
  ...
  <h3>Extreme markdown outliers (IQR method, per-record)</h3>
  {_records_to_html_table(micro_inspection['extreme_markdown_outliers'])}

  <h3>Largest week-over-week sales anomalies, per Store/Dept</h3>
  {_records_to_html_table(micro_inspection['date_level_anomalies'])}
</div>
...
def render_insights_markdown(
    ...
    lines.append("- **Extreme markdown outliers (record-level):**")
    for row in micro_inspection["extreme_markdown_outliers"][:5]:
        lines.append(
            f"  - {row['column']} = {row['value']:.2f} at Store {row['Store']}, Dept {row['Dept']}, "
            f"{row['Date']} (upper bound {row['upper_bound']:.2f})"
        )
    lines.append("- **Largest week-over-week anomalies:**")
    for row in micro_inspection["date_level_anomalies"][:5]:
        lines.append(
    ...
    if markdown_effect:
        lines.append(
            f"- Mean sales in weeks with any markdown active: "
            f"{markdown_effect.get('mean_sales_weeks_with_any_markdown')} vs without: "
            f"{markdown_effect.get('mean_sales_weeks_without_markdown')} "
            f"({markdown_effect.get('extreme_outlier_count')} extreme markdown outliers detected)."
        )
...
def _build_crew(profiling: dict[str, Any], micro_inspection: dict[str, Any], business_intelligence: dict[str, Any], contract: dict[str, Any]):
    ...
    profiler_task = Task(
        description=(
            ...
            "single most notable week-over-week anomaly.\n\nPROFILING:\n"
    ...
    bi_task = Task(
        description=(
            ...
            "citing at least one specific store from the holiday-spike findings.\n\nBUSINESS_INTELLIGENCE:\n"
